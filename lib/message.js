...
const DURATION_TOAST = 3200;
const DURATION_WARNING = 4800;
...
function toast(text) {
    return notify(text, COLOR_BACKGROUND_TOAST, DURATION_TOAST);
}

function warning(text) {
    return notify(text, COLOR_BACKGROUND_WARNING, DURATION_WARNING);
}

function error(text) {
    return notify(text, COLOR_BACKGROUND_ERROR, 0);
}

function progress(text) {
    return notify(text, COLOR_BACKGROUND_PROGRESS, 0);
}

const items = [];

function notify(text, background, duration) {
    const content = document.createElement('div');
    ...
    if (duration > 0) {
        item.timer = setTimeout(close, duration);
    }
